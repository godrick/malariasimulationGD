...
.scal_one_warmup_pfpr <- function(init_EIR,
  ...
  if (length(data_list) != nrow(as.matrix(setup$D))) {
    stop(sprintf(
      "Calibration probe returned %d node outputs, expected %d from setup$D.",
      length(data_list), nrow(as.matrix(setup$D))
    ), call. = FALSE)
  }
  ...
  combined <- msimGD_bind_node_outputs(data_list)
  ...
  if (nrow(meas) != measurement_window_days) {
    stop(sprintf(
      "Calibration measurement window expected %d days but found %d.",
      measurement_window_days, nrow(meas)
    ), call. = FALSE)
  }
  list(
    init_EIR     = init_EIR,
