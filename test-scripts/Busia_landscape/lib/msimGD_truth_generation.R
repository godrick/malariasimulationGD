...
msimGD_make_base_parameters <- function(NH, theta, individual_mosquitoes = TRUE) {
  if (!is.numeric(NH) || length(NH) != 1L || !is.finite(NH) || NH <= 0) {
    stop("NH must be a single positive finite human population size.",
         call. = FALSE)
  }
  for (nm in c("nE", "nL", "nP")) {
    if (is.null(theta[[nm]]) ||
        length(theta[[nm]]) != 1L ||
        is.na(as.integer(theta[[nm]])) ||
        as.integer(theta[[nm]]) < 1L) {
      stop(sprintf("theta$%s must be a positive integer.", nm), call. = FALSE)
    }
  }
  if (!is.null(theta$nEIP) &&
      (length(theta$nEIP) != 1L ||
       is.na(as.integer(theta$nEIP)) ||
       as.integer(theta$nEIP) < 1L)) {
    stop("theta$nEIP must be a positive integer when supplied.", call. = FALSE)
  }
  ...
  if (!is.null(theta$nEIP)) {
    bp$native_mosquito_nEIP <- theta$nEIP
  }
  if (!is.null(theta$nu)) {
...
msimGD_bind_node_outputs <- function(data_list, add_node = FALSE, arm_label = NULL) {
  if (!is.list(data_list) ||
      length(data_list) < 1L ||
      !all(vapply(data_list, is.data.frame, logical(1)))) {
    stop("data_list must be a non-empty list of data.frames.", call. = FALSE)
  }
  all_names <- unique(unlist(lapply(data_list, names), use.names = FALSE))
  rows <- lapply(seq_along(data_list), function(i) {
    d <- data_list[[i]]
  ...
  front <- intersect(c("node", "arm", "timestep"), names(out))
...
.msimGD_prepare_truth_run <- function(setup, cube, NF, NH, tmax, mu, p_move, release,
  ...
  if (length(NH) != n_nodes ||
      any(!is.finite(as.numeric(NH))) ||
      any(as.numeric(NH) <= 0)) {
    stop(sprintf(
      "NH must be a positive scalar or a positive vector matching the %d landscape nodes.",
      n_nodes
    ), call. = FALSE)
  }
  ...
  if (!is.null(NF) &&
      (length(NF) != n_nodes ||
       any(!is.finite(as.numeric(NF))) ||
       any(as.numeric(NF) <= 0))) {
    stop(sprintf(
      "NF must be NULL, a positive scalar, or a positive vector matching the %d landscape nodes.",
      n_nodes
    ), call. = FALSE)
  }
