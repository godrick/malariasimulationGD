...
script_path <- if (length(file_arg) > 0L) {
  ...
  normalizePath("test-scripts/Busia_landscape/careful/03_plot_careful.R",
                mustWork = TRUE)
}
...
if (!identical(as.integer(sum(nodes$NH_per_node)), as.integer(expected_total_population))) {
  stop(sprintf(
    paste(
      "context.rds was built with total population %d,",
      "but the current Busia input total is %d.",
      "Rerun the careful workflow from 00_calibrate_init_eir.R."
    ),
    sum(nodes$NH_per_node), expected_total_population
  ), call. = FALSE)
}
...
if (is.null(target_prev) ||
    !isTRUE(all.equal(as.numeric(target_prev),
                      expected_target_pfpr, tolerance = 1e-12))) {
  stop(sprintf(
    paste(
    ...
    if (is.null(target_prev)) "NA" else sprintf("%.3f", target_prev),
    expected_target_pfpr
  ), call. = FALSE)
}
