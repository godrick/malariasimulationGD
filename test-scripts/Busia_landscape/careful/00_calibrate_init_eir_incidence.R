#!/usr/bin/env Rscript
# ------------------------------------------------------------------------------
# 00_calibrate_init_eir_incidence.R
# ------------------------------------------------------------------------------
# Careful-workflow alternative stage 0: calibrate raw counterfactual
# no-intervention init_EIR and the synthetic contact-covariate coefficient delta
# to match modeled infection incidence in children aged 6 months to younger
# than 10 years after baseline ITNs and ACT clinical treatment are applied at
# runtime.
#
# Target summary across the 58 Busia sites:
#   mean incidence = 3.138 infections/person-year
#   site-level SD  = 1.381 infections/person-year
#   hazard CV      = 0.48
...
#
# The simulator exposes modeled infection incidence and microscopy prevalence as
# separate renderers. It does not expose a microscopy-confirmed infection
# incidence counter, so this script labels outputs as modeled infection
# incidence rather than microscopy-confirmed incidence.
#
# Main outputs, under BUSIA_CAREFUL_OUT_DIR or output/careful:
#   calibrated_init_eir_incidence.rds
#   incidence_calibration_log.csv
#   incidence_calibration_node_summary.csv
#   incidence_calibration_global_summary.csv
#   covariate_delta_search.csv
#   incidence_calibration_no_intervention_node_summary.csv
#   incidence_calibration_no_intervention_global_summary.csv
#   theta.rds
#
# By default this script does not overwrite calibrated_init_eir.rds, because the
# downstream careful workflow currently validates a PfPR target. Set
# BUSIA_WRITE_STAGE00_COMPAT=true to also write a compatibility artifact.
# ------------------------------------------------------------------------------
...
script_path <- if (length(file_arg) > 0L) {
  ...
  normalizePath(
    "test-scripts/Busia_landscape/careful/00_calibrate_init_eir_incidence.R",
    mustWork = TRUE
  )
}
...
parse_numeric_vector_env <- function(name, default) {
  ...
  if (length(out) < 1L || any(!is.finite(out))) {
    stop(sprintf(
      "%s must be a comma- or space-separated numeric vector.", name
    ), call. = FALSE)
  }
  as.numeric(out)
}
...
objective_value <- function(mean_incidence, sd_incidence, cv_incidence,
                            targets, mean_weight, sd_weight, cv_weight) {
  mean_term <- ((mean_incidence - targets$mean) / targets$mean)^2
  sd_term <- ((sd_incidence - targets$sd) / targets$sd)^2
  cv_term <- ((cv_incidence - targets$cv) / targets$cv)^2
  mean_weight * mean_term + sd_weight * sd_term + cv_weight * cv_term
}

summarise_incidence_run <- function(data_list, nodes, covariate,
                                    measurement_window_days, age_band_days,
                                    init_EIR, delta, kind, targets,
                                    mean_weight, sd_weight, cv_weight) {
  ...
  objective <- objective_value(
    mean_incidence = unweighted_mean,
    sd_incidence = unweighted_sd,
    cv_incidence = unweighted_cv,
    targets = targets,
  ...
  global_summary <- data.frame(
    ...
    covariate_delta = as.numeric(delta),
    ...
    objective = objective,
    objective_mean_weight = mean_weight,
    objective_sd_weight = sd_weight,
    objective_cv_weight = cv_weight,
...
write_best_outputs <- function(best, out_dir, targets, metadata,
                               candidate_log, delta_log) {
  if (is.null(best)) {
    return(invisible(FALSE))
  }
  utils::write.csv(best$node_summary,
                   file.path(out_dir, "incidence_calibration_node_summary.csv"),
                   row.names = FALSE)
  utils::write.csv(best$global_summary,
                   file.path(out_dir, "incidence_calibration_global_summary.csv"),
                   row.names = FALSE)
  if (length(candidate_log) > 0L) {
    utils::write.csv(do.call(rbind, candidate_log),
                     file.path(out_dir, "incidence_calibration_log.csv"),
                     row.names = FALSE)
  }
  if (length(delta_log) > 0L) {
    utils::write.csv(do.call(rbind, delta_log),
                     file.path(out_dir, "covariate_delta_search.csv"),
                     row.names = FALSE)
  }
  if (!is.null(best$no_intervention_counterfactual)) {
    utils::write.csv(
      best$no_intervention_counterfactual$node_summary,
      file.path(out_dir, "incidence_calibration_no_intervention_node_summary.csv"),
      row.names = FALSE
    )
    utils::write.csv(
      best$no_intervention_counterfactual$global_summary,
      file.path(out_dir, "incidence_calibration_no_intervention_global_summary.csv"),
      row.names = FALSE
    )
  }
  saveRDS(
    c(
      list(
        ...
        realised_eir = best$global_summary$mean_realized_eir,
        ...
        search_log = if (length(candidate_log) > 0L) do.call(rbind, candidate_log) else NULL,
        delta_search = if (length(delta_log) > 0L) do.call(rbind, delta_log) else NULL,
        no_intervention_counterfactual =
          best$no_intervention_counterfactual
      ),
      metadata
    ),
    file.path(out_dir, "calibrated_init_eir_incidence.rds")
  )
  invisible(TRUE)
}
  ...
  saveRDS(
    c(
      list(
        ...
        raw_counterfactual_init_EIR = best$init_EIR,
        theta = theta,
        calibration_target = "modeled_infection_incidence_182_3650",
        calibrated_eir_definition =
          best$global_summary$calibrated_eir_definition,
...
MEAN_TOLERANCE_REL <- parse_numeric_env(
  "BUSIA_INCIDENCE_MEAN_TOLERANCE_REL", 0.05, positive = TRUE
)
OBJECTIVE_MEAN_WEIGHT <- parse_numeric_env(
  "BUSIA_OBJECTIVE_MEAN_WEIGHT", 6, non_negative = TRUE
)
OBJECTIVE_SD_WEIGHT <- parse_numeric_env(
  "BUSIA_OBJECTIVE_SD_WEIGHT", 2, non_negative = TRUE
)
OBJECTIVE_CV_WEIGHT <- parse_numeric_env(
  "BUSIA_OBJECTIVE_CV_WEIGHT", 0.5, non_negative = TRUE
)
...
DELTA_GRID <- sort(unique(parse_numeric_vector_env(
  "BUSIA_DELTA_GRID", seq(0, 1.2, by = 0.2)
)))
if (any(DELTA_GRID < 0)) {
  stop("BUSIA_DELTA_GRID values must be non-negative.", call. = FALSE)
}
...
DELTA_REFINE_STEP <- parse_numeric_env(
  "BUSIA_DELTA_REFINE_STEP", 0.1, non_negative = TRUE
)
...
cat(sprintf(
  "[%s] careful/00 incidence: target mean %.3f, SD %.3f, CV %.3f, age %d-%d days\n",
  format(Sys.time(), "%F %T"),
  targets$mean, targets$sd, targets$cv,
...
cat("Calibration EIR definition: raw counterfactual no-intervention init_EIR; interventions are applied after equilibrium initialization.\n")
cat(sprintf(
...
base_covariate_settings <- seven_node_covariate_settings()
...
metadata <- list(
  ...
  objective_sd_weight = OBJECTIVE_SD_WEIGHT,
  objective_cv_weight = OBJECTIVE_CV_WEIGHT,
  base_covariate_settings = base_covariate_settings,
...
make_covariate <- function(delta) {
  settings <- base_covariate_settings
  settings$delta <- as.numeric(delta)
  build_seven_node_covariate(setup$D, settings = settings)
}

run_candidate <- function(delta, init_EIR, kind) {
  if (!can_start_candidate()) {
    budget_limited <<- TRUE
    return(NULL)
  }

  cov <- make_covariate(delta)
  ...
  cat(sprintf(
    "  [%s] delta=%5.3f raw_init_EIR=%8.4f ...\n",
    kind, delta, init_EIR
  ))
  t0 <- Sys.time()
  ...
  summary <- summarise_incidence_run(
    ...
    sd_weight = OBJECTIVE_SD_WEIGHT,
    cv_weight = OBJECTIVE_CV_WEIGHT
  )
  ...
  if (is.null(best) || global$objective < best$global_summary$objective) {
    best <<- list(
  ...
  } else if (length(candidate_log) > 0L) {
    utils::write.csv(do.call(rbind, candidate_log),
                     file.path(out_dir, "incidence_calibration_log.csv"),
                     row.names = FALSE)
  }
...
run_no_intervention_counterfactual <- function(best) {
  ...
  cat(sprintf(
    "  [selected_no_intervention_counterfactual] delta=%5.3f raw_init_EIR=%8.4f ...\n",
    best$delta,
    best$init_EIR
  ))
  t0 <- Sys.time()
  ...
  summary <- summarise_incidence_run(
    ...
    init_EIR = best$init_EIR,
    delta = best$delta,
    kind = "selected_no_intervention_counterfactual",
    targets = targets,
    mean_weight = OBJECTIVE_MEAN_WEIGHT,
    sd_weight = OBJECTIVE_SD_WEIGHT,
    cv_weight = OBJECTIVE_CV_WEIGHT
  ...
  cat(sprintf(
    "       no-intervention mean=%.3f sd=%.3f cv=%.3f eir=%.3f range=[%.3f, %.3f] [%.1f s]\n",
    ...
    summary$global_summary$mean_realized_eir,
...
candidate_already_run <- function(delta, init_EIR) {
  if (length(candidate_log) < 1L) {
    return(FALSE)
  }
  log_df <- do.call(rbind, candidate_log)
  any(abs(log_df$covariate_delta - delta) < 1e-12 &
        abs(log_df$init_EIR - init_EIR) / pmax(init_EIR, 1) < 1e-8)
}
...
evaluate_delta <- function(delta, phase_label) {
  cat(sprintf("[%s] evaluating delta %.3f (%s)\n",
              format(Sys.time(), "%F %T"), delta, phase_label))
  rows_before <- length(candidate_log)

  start_eir <- if (!is.null(best)) best$init_EIR else EIR_START
  start_eir <- min(max(start_eir, EIR_MIN), EIR_MAX)
  if (!candidate_already_run(delta, start_eir)) {
    run_candidate(delta, start_eir, sprintf("%s_start", phase_label))
  }
  ...
  for (it in seq_len(MAX_BRACKET_ITER)) {
    ...
    log_df <- current_log()
    rows <- log_df[abs(log_df$covariate_delta - delta) < 1e-12, , drop = FALSE]
    if (!is.null(find_bracket(rows))) {
      break
    }
    next_eir <- suggest_next_eir(rows)
    if (!is.finite(next_eir) || next_eir <= 0 ||
        candidate_already_run(delta, next_eir)) {
      break
    }
    run_candidate(delta, next_eir, sprintf("%s_bracket_%d", phase_label, it))
  }
  ...
  for (it in seq_len(MAX_REFINE_ITER)) {
    ...
    best_mean_row <- rows[which.min(abs(
      rows$unweighted_mean_incidence_per_person_year - targets$mean
    )), , drop = FALSE]
    rel_err <- abs(best_mean_row$unweighted_mean_incidence_per_person_year -
                     targets$mean) / targets$mean
    if (rel_err <= MEAN_TOLERANCE_REL) {
      break
    }
    next_eir <- suggest_next_eir(rows)
    if (!is.finite(next_eir) || next_eir <= 0 ||
    ...
    run_candidate(delta, next_eir, sprintf("%s_refine_%d", phase_label, it))
  }
  ...
  rows <- log_df[abs(log_df$covariate_delta - delta) < 1e-12, , drop = FALSE]
  if (nrow(rows) > 0L) {
    selected <- rows[which.min(rows$objective), , drop = FALSE]
    selected$phase <- phase_label
    selected$n_candidates_for_delta <- nrow(rows)
    selected$new_candidates_for_delta <- length(candidate_log) - rows_before
    delta_log[[length(delta_log) + 1L]] <<- selected
    utils::write.csv(do.call(rbind, delta_log),
                     file.path(out_dir, "covariate_delta_search.csv"),
                     row.names = FALSE)
  }
...
for (delta in DELTA_GRID) {
  ...
  evaluate_delta(delta, "grid")
}

if (!budget_limited && length(delta_log) > 0L && DELTA_REFINE_STEP > 0) {
  delta_df <- do.call(rbind, delta_log)
  best_delta <- delta_df$covariate_delta[which.min(delta_df$objective)]
  refine_grid <- sort(unique(pmax(0, c(
    best_delta - DELTA_REFINE_STEP,
    best_delta + DELTA_REFINE_STEP
  ))))
  refine_grid <- setdiff(round(refine_grid, 10), round(DELTA_GRID, 10))
  if (length(refine_grid) > 0L) {
    cat(sprintf("Local delta refinement around %.3f: %s\n",
                best_delta, paste(refine_grid, collapse = ", ")))
    for (delta in refine_grid) {
      if (budget_limited) {
        break
      }
      evaluate_delta(delta, "local")
    }
  }
}

if (is.null(best)) {
  stop("No incidence calibration candidates were evaluated.", call. = FALSE)
}
...
cat(sprintf(
  "[%s] incidence calibration %s in %.1f s (%.1f min).\n",
  format(Sys.time(), "%F %T"),
...
cat(sprintf(
  "Selected delta = %.3f | raw_init_EIR = %.4f | intervention mean = %.3f | SD = %.3f | CV = %.3f | realised EIR = %.3f | objective = %.4f\n",
  best$delta,
  best$init_EIR,
  ...
  best$global_summary$mean_realized_eir,
  best$global_summary$objective
))
if (!is.null(best$no_intervention_counterfactual)) {
  cat(sprintf(
    "Selected raw_init_EIR no-intervention counterfactual mean incidence = %.3f (same EIR and delta, no ITNs/ACT).\n",
    best$no_intervention_counterfactual$global_summary$unweighted_mean_incidence_per_person_year
  ))
}
cat(sprintf("Wrote incidence calibration outputs to: %s\n", out_dir))
if (!WRITE_STAGE00_COMPAT) {
