...
script_path <- if (length(file_arg) > 0L) {
  ...
  normalizePath("test-scripts/Busia_landscape/careful/02_release_from_checkpoint.R",
                mustWork = TRUE)
}
...
expected_total_population <- busia_default_total_population()
if (!identical(as.integer(sum(NH_per_node)), as.integer(expected_total_population))) {
  stop(sprintf(
    paste(
      "context.rds was built with total population %d,",
      "but the current Busia input total is %d.",
      "Rerun 00_calibrate_init_eir.R and 01_warmup_and_checkpoint.R",
      "with the current population settings."
    ),
    sum(NH_per_node), expected_total_population
  ), call. = FALSE)
}
expected_target_pfpr <- busia_default_target_pfpr()
if (is.null(context$target_prevalence) ||
    !isTRUE(all.equal(as.numeric(context$target_prevalence),
                      expected_target_pfpr, tolerance = 1e-12))) {
  stop(sprintf(
    paste(
      "context.rds was built with target PfPR %s,",
      "but the current Busia target PfPR is %.3f.",
      "Rerun 00_calibrate_init_eir.R and 01_warmup_and_checkpoint.R",
      "with the current target."
    ),
    if (is.null(context$target_prevalence)) "NA" else sprintf("%.3f", context$target_prevalence),
    expected_target_pfpr
  ), call. = FALSE)
}
...
allowed_mat <- busia_allowed_matrix(n_nodes)
...
validate_busia_simulation_output(
  out_release$res,
  n_nodes = n_nodes,
  required_columns = c("timestep", "n_detect_lm_730_3650", "n_age_730_3650", "n_infections"),
  require_genotypes = TRUE,
  label = "release arm"
)
validate_busia_simulation_output(
  out_no_release$res,
...
stack_data <- function(res, arm_label) {
  ...
  msimGD_bind_node_outputs(data_list, add_node = TRUE, arm_label = arm_label)
}
...
for (i in seq_len(n_nodes)) {
  ...
  if (is.null(female) || is.null(male)) {
    stop(sprintf("Release output for node %d is missing mosquito genotype counts.", i),
         call. = FALSE)
  }
  H_genos <- intersect(H_genos_definition, colnames(female))
  if (length(H_genos) < 1L) {
    stop(sprintf(
      "Release output for node %d has no H-carrier genotype columns. Available female genotypes: %s",
      i, paste(colnames(female), collapse = ", ")
    ), call. = FALSE)
  }
...
if (all(is.na(first_H_per_node))) {
  stop("No H-carrier mosquitoes appeared in any node after the configured release.",
       call. = FALSE)
}
...
summary_df <- data.frame(
  ...
  target_prevalence       = context$target_prevalence,
  tmax_release            = tmax_release,
  release_day             = release_day,
  horizon_day             = horizon_day,
  readout_day             = readout_day,
  n_nodes                 = n_nodes,
  total_human_population  = sum(NH_per_node),
