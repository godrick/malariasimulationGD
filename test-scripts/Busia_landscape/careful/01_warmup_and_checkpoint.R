...
script_path <- if (length(file_arg) > 0L) {
  ...
  normalizePath("test-scripts/Busia_landscape/careful/01_warmup_and_checkpoint.R",
                mustWork = TRUE)
}
...
if (is.null(cal$target_prevalence) ||
    !isTRUE(all.equal(as.numeric(cal$target_prevalence),
                      expected_target_pfpr, tolerance = 1e-12))) {
  stop(sprintf(
    paste(
      "calibrated_init_eir.rds was built with target PfPR %s,",
      "but the current Busia target PfPR is %.3f.",
      "Rerun 00_calibrate_init_eir.R with the current target."
    ),
    if (is.null(cal$target_prevalence)) "NA" else sprintf("%.3f", cal$target_prevalence),
...
land_total_population <- if (!is.null(cal$NH_per_node)) {
  sum(as.integer(cal$NH_per_node))
} else {
  busia_default_total_population()
}
expected_total_population <- busia_default_total_population()
if (!identical(as.integer(land_total_population), as.integer(expected_total_population))) {
  stop(sprintf(
    paste(
      "calibrated_init_eir.rds was built with total population %d,",
      "but the current Busia input total is %d.",
      "Rerun 00_calibrate_init_eir.R with the current population settings."
    ),
    land_total_population, expected_total_population
  ), call. = FALSE)
}
land  <- build_busia_landscape(total_population = land_total_population)
...
if (length(library_obj$snapshots) != N_SNAPSHOTS) {
  stop("Checkpoint library did not produce the requested number of snapshots.",
       call. = FALSE)
}
...
saveRDS(list(
  ...
  target_prevalence  = cal$target_prevalence,
  theta              = theta,
  seasonality        = seas,
  seed               = RNG_SEED,
