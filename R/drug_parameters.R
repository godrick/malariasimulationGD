...
# Use the same treatment regime when solving and instantiating an equilibrium
# baseline so the initial human state matches the equilibrium FOIM target.
equilibrium_treatment_coverage <- function(parameters) {
  sum(get_treatment_coverages(parameters, 1L))
}
