  ...
  function(timestep) {
    ...
    for (i in seq_along(solvers)) {
      if (native_mosquito_backend_enabled(parameters)) {
        species_M <- sum(solvers[[i]]$get_summary()$totals[c("Sm", "Pm", "Im")])
      } else {
...
final_total_M_from_components <- function(solvers, parameters, variables = NULL) {
  if (legacy_individual_mosquito_backend_enabled(parameters)) {
    if (is.null(variables) || is.null(variables$mosquito_state) || is.null(variables$species)) {
      stop("legacy individual mosquito totals require mosquito variables.", call. = FALSE)
    }

    adult_states <- variables$mosquito_state$get_index_of(c("Sm", "Pm", "Im"))
    return(sum(vapply(
      parameters$species,
      function(species_name) {
        species_index <- variables$species$get_index_of(species_name)
        adult_states$copy()$and(species_index)$size()
      },
      numeric(1)
    )))
  }
...
final_total_M_by_node <- function(solvers, parameters, variables = NULL) {
  if (!is.list(parameters)) {
    stop("parameters must be a non-empty list of per-node parameter sets.", call. = FALSE)
  }
  if (!is.list(solvers) || length(solvers) != length(parameters)) {
    stop("solvers must be a list aligned with the metapopulation parameters.", call. = FALSE)
  }

  vapply(
    seq_along(parameters),
    function(i) {
      node_variables <- if (!is.null(variables)) variables[[i]] else NULL
      final_total_M_from_components(
        solvers = solvers[[i]],
