...
prob_bitten <- function(
  ...
  if (parameters$bednets) {
    ...
    time_offset <- if (!is.null(parameters$vector_control_time_offset)) {
      as.integer(parameters$vector_control_time_offset)
    } else {
      0L
    }
    since_net <- (timestep + time_offset) - net_time
  ...
  if (parameters$spraying) {
    ...
    since_spray <- (timestep + time_offset) - spray_time
...
distribute_nets <- function(variables, throw_away_net, parameters, correlations) {
  function(timestep) {
    ...
    if (length(parameters$bednet_skip_timesteps) > 0L) {
      matches <- matches & !(parameters$bednet_timesteps %in% parameters$bednet_skip_timesteps)
    }
    if (any(matches)) {
      target <- which(sample_intervention(
        seq(parameters$human_population),
